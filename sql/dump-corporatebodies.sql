...
DB.DBA.XML_SET_NS_DECL('concept-status', 'http://publications.europa.eu/resource/authority/concept-status/', 1);
DB.DBA.XML_SET_NS_DECL('label-type', 'http://publications.europa.eu/resource/authority/label-type/', 1);
DB.DBA.XML_SET_NS_DECL('eurovoc-schema', 'http://eurovoc.europa.eu/schema#', 1);
DB.DBA.XML_SET_NS_DECL('script', 'http://publications.europa.eu/resource/authority/script/', 1);
...
CREATE DUMP_PROCEDURE(dump_corpbody,
SPARQL
DEFINE input:storage ""
PREFIX delta: <http://www.w3.org/2004/delta#>
SELECT ?s ?p ?o
FROM <http://publications.europa.eu/resource/authority/corporatebodies/>
WHERE {
	?s ?p ?o .
}
);
