...
SET u{STAGE} bloc-staging;
SET u{DIFFG} bloc-diff;

SPARQL CREATE SILENT GRAPH <$u{STAGE}>;
SPARQL CLEAR GRAPH <$u{STAGE}>;

DELETE FROM DB.DBA.LOAD_LIST WHERE ll_file = '/home/freundt/author/region/wd-bloc.ttl.repl';
ld_add('/home/freundt/author/region/wd-bloc.ttl.repl', '$u{STAGE}');
rdf_loader_run();
CHECKPOINT;
